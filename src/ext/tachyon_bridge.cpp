...
static GUICompoundArgs parse_gui_args(JSContext *ctx, int argc, JSValueConst *argv)
{
    ...
    if (argc >= 1)
        JS_ToFloat64(ctx, (double *)&args.x, argv[0]);
    if (argc >= 2)
        JS_ToFloat64(ctx, (double *)&args.y, argv[1]);
    if (argc >= 3)
        JS_ToFloat64(ctx, (double *)&args.w, argv[2]);
    if (argc >= 4)
        JS_ToFloat64(ctx, (double *)&args.h, argv[3]);

    // Primary color (r,g,b,a)
    if (argc >= 5)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[4]);
        args.r1 = (float)v;
    }
    if (argc >= 6)
    {
    ...
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[6]);
        args.b1 = (float)v;
    }
    if (argc >= 8)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[7]);
        args.a1 = (float)v;
    }
    ...
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[8]);
        args.r2 = (float)v;
    }
    if (argc >= 10)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[9]);
        args.g2 = (float)v;
    }
    if (argc >= 11)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[10]);
        args.b2 = (float)v;
    }
    if (argc >= 12)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[11]);
        args.a2 = (float)v;
    }
    ...
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[12]);
        args.r3 = (float)v;
    }
    if (argc >= 14)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[13]);
        args.g3 = (float)v;
    }
    if (argc >= 15)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[14]);
        args.b3 = (float)v;
    }
    if (argc >= 16)
    {
        double v;
        JS_ToFloat64(ctx, &v, argv[15]);
        args.a3 = (float)v;
    }
...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "GUI.loadFont(path, size?)");
    const char *path = JS_ToCString(ctx, argv[0]);
    if (!path)
        return JS_EXCEPTION;
